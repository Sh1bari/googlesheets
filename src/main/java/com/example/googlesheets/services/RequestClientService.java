...
public class RequestClientService {
    ...
    private final HashMap<String, MultiLoginInfoVar> multiLoginInfoVar = new HashMap<>(); //ключ - имя
    ...
    public String getInfoFromMultiLogin(String account, String credentialsAccount, String credentialsToken, String time) throws JsonProcessingException {
        ...
        ResponseEntity<String> res = postRequest("https://platform.engageya.com/dashboard-api/api/reports/v1/reports/currentlyRunningCampaign",
                KEITARO_TOKEN,
                MULTILOGIN_BODY(time),
                credentialsAccount,
                credentialsToken);

        JsonNode jsonNodeSpent = objectMapper.readTree(res.getBody());
        JsonNode rowsNodeSpent = jsonNodeSpent.get("data").get("tableData");

        List<List<String>> listSpent = objectMapper.readValue(
                rowsNodeSpent.toString(),
                new TypeReference<List<List<String>>>() {
                }
        );

        listSpent.forEach(o -> {
            if(multiLoginInfoVar.containsKey(o.get(0))) {
                ...
                if(keitaroInfo.get(var.getCreativeId()) != null) {
                    ...
                    if(o.get(0).contains("DE")){
                        maxLeadCost = "165";
                    }else if(o.get(0).contains("AT")){
                        maxLeadCost = "160";
                    }else if(o.get(0).contains("UK")){
                        maxLeadCost = "100";
                    } else {
                        maxLeadCost = "Error";
                    }
